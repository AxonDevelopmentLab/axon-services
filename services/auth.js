    ...
    login: async () => {
      ...
      if (getAccountDetails.Password !== password) {
        ...
        if (RequestData.getdatafrom && RequestData.getdatafrom.length > 0) {
          require('./start_services.js').getData(getAccountDetails.ID, RequestData.getdatafrom, RequestData.security || false).then((allDatas) => {
            if (allDatas?.status && allDatas.status === 400) return UserHTTP.send(allDatas);
            return UserHTTP.send({ status: 200, Token: getAccountDetails.Token, getdatafrom: allDatas }); 
          });
        } else {
          return UserHTTP.send({ status: 200, Token: getAccountDetails.Token }) 
        }
      }
    },
    register: async () => {
      ...
      UserHTTP.send({ status: 200, Token: generateAccountToken });
